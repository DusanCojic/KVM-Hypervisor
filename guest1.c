...
_start(void) {
	...
	int fd = fopen("file.txt", 1); // open file for read/write
	if (fd >= 0) {
		char wb1[256] = "Write to local file\n";
		fd = fwrite(fd, wb1, strlen(wb1));

		char wb2[256] = "Write to local file again\n";
		fd = fwrite(fd, wb2, strlen(wb2));

		char wb3[256] = "Write to local file again and again\n";
		fd = fwrite(fd, wb3, strlen(wb3));
